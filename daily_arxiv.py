...
def get_daily_papers(topic, query="slam", max_results=2):
    ...
    for result in search_engine.results():
        ...
        daily_json = {'paper_title': paper_title,
                      #   'paper_id':paper_id,
                      #   'paper_url':paper_url,
                      #   'code_url':code_url,
        ...
        daily_json['paper_url'] = paper_url
        daily_json['paper_id'] = paper_key
        ...
        try:
            ...
            daily_papers[paper_key] = daily_json  # white paper_id paper_key
    ...
    return data, data_web, daily_papers
...
def get_today_papers(yesterday_json_path, today_json_path, today_papers):
    ...
    with open(yesterday_json_path, "r") as f:
        ...
        if not content:
            ...
            m = json.loads(content)
    yesterday_json = m.copy()
    today_json = {}
    for key in today_papers.keys():
        for keyword in today_papers[key].keys():
            paper = today_papers[key][keyword]
            if key not in yesterday_json:
                yesterday_json[key] = {}
            if keyword not in yesterday_json[key].keys():
                if key not in today_json:
                    today_json[key] = {}
                today_json[key][keyword] = paper

    with open(today_json_path, "w") as f:
        json.dump(today_json, f)

    return bool(today_json), today_json
